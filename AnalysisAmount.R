...
stratified_double <- function(var, p) {
  obs  <- seq_along(var)
  groups <- unique(var)
  indices <- numeric()
  for(g in groups) {
...
misClasificError_lr_double <- NULL
misClasificError_lr_triple <- NULL
misClasificError_lr_quadruple <- NULL
misClasificError_rf_double <- NULL
misClasificError_rf_triple <- NULL
misClasificError_rf_quadruple <- NULL

auc_lr_double <- NULL
auc_lr_triple <- NULL
auc_lr_quadruple <- NULL
auc_rf_double <- NULL
auc_rf_triple <- NULL
auc_rf_quadruple <- NULL

aupr_lr_double <- NULL
aupr_lr_triple <- NULL
aupr_lr_quadruple <- NULL
aupr_rf_double <- NULL
aupr_rf_triple <- NULL
aupr_rf_quadruple <- NULL

mcc_lr_double <- NULL
mcc_lr_triple <- NULL
mcc_lr_quadruple <- NULL
mcc_rf_double <- NULL
mcc_rf_triple <- NULL
mcc_rf_quadruple <- NULL
...
for (i in 1:50){
  ...
  set.seed(seed+i)
  train_ind <- stratified_double(dataset[,dependent_variable], 0.7)
  
  train <- dataset[train_ind, ]
  test <- dataset[-train_ind, ]
  
  double <- DAColNoise(same_noise = FALSE,     # "same" = TRUE or "different" = FALSE
  ...
  triple <- DAColNoise(same_noise = FALSE,     # "same" = TRUE or "different" = FALSE
                          tbl_columns_selected = columns,   # Indices of the selected columns
                          dataset = train, 
                          tbl_classification_columns_selected = dependent_variable,   # Index of the column of the dependent variable
                          balanced_classes = balance_class,   # Name of the classes that should be balanced in vector 
  ...
  quadruple <- DAColNoise(same_noise = FALSE,     # "same" = TRUE or "different" = FALSE
                       tbl_columns_selected = columns,   # Indices of the selected columns
                       dataset = train, 
                       tbl_classification_columns_selected = dependent_variable,   # Index of the column of the dependent variable
                       balanced_classes = balance_class,   # Name of the classes that should be balanced in vector 
  ...
  # Train model #
  model_double <- glm(as.formula(paste(names(double)[dependent_variable], "~.")), family = binomial, data = double)
  model_triple <- glm(as.formula(paste(names(triple)[dependent_variable], "~.")), family = binomial, data = triple)
  model_quadruple <- glm(as.formula(paste(names(quadruple)[dependent_variable], "~.")), family = binomial, data = quadruple)
  
  
  # predict # # AUC, PR, MCC #
  
  fitted_results_double <- predict(model_double,newdata=subset(test,select=c(-dependent_variable)),type='response')
  fitted_results_double <- ifelse(fitted_results_double > 0.5,1,0)
  misClasificError_lr_double[i] <- mean(fitted_results_double != test[,dependent_variable])
  roc_double_lr <- tryCatch({pROC::roc(fitted_results_double, test[,dependent_variable])},
                            error=function(cond){return(NA)})
  auc_lr_double[i] <- tryCatch({pROC::auc(roc_double_lr)[1]}, error=function(cond){return(NA)})
  aupr_lr_double[i] <- PRROC::pr.curve(as.factor(fitted_results_double), test[,dependent_variable])$auc.integral
  mcc_lr_double[i] <- mltools::mcc(as.factor(fitted_results_double), as.factor(test[,dependent_variable]))
  
  
  fitted_results_triple <- predict(model_triple,newdata=subset(test,select=c(-dependent_variable)),type='response')
  fitted_results_triple <- ifelse(fitted_results_triple > 0.5,1,0)
  misClasificError_lr_triple[i] <- mean(fitted_results_triple != test[,dependent_variable])
  roc_triple_lr <- tryCatch({pROC::roc(fitted_results_triple, test[,dependent_variable])},
                               error=function(cond){return(NA)})
  auc_lr_triple[i] <- tryCatch({pROC::auc(roc_triple_lr)[1]}, error=function(cond){return(NA)})
  aupr_lr_triple[i] <- PRROC::pr.curve(as.factor(fitted_results_triple), test[,dependent_variable])$auc.integral
  mcc_lr_triple[i] <- mltools::mcc(as.factor(fitted_results_triple), as.factor(test[,dependent_variable]))
  
  
  fitted_results_quadruple <- predict(model_quadruple,newdata=subset(test,select=c(-dependent_variable)),type='response')
  fitted_results_quadruple <- ifelse(fitted_results_quadruple > 0.5,1,0)
  misClasificError_lr_quadruple[i] <- mean(fitted_results_quadruple != test[,dependent_variable])
  roc_quadruple_lr <- tryCatch({pROC::roc(fitted_results_quadruple, test[,dependent_variable])},
                            error=function(cond){return(NA)})
  auc_lr_quadruple[i] <- tryCatch({pROC::auc(roc_quadruple_lr)[1]}, error=function(cond){return(NA)})
  aupr_lr_quadruple[i] <- PRROC::pr.curve(as.factor(fitted_results_quadruple), test[,dependent_variable])$auc.integral
  mcc_lr_quadruple[i] <- mltools::mcc(as.factor(fitted_results_quadruple), as.factor(test[,dependent_variable]))
  ...
  double[,dependent_variable] <- as.factor(double[,dependent_variable])
  rf_double <- randomForest(as.formula(paste(names(double)[dependent_variable], "~.")), data = double, ntree=100, mtry=2, importance=TRUE)
  
  triple <- as.data.frame(triple)
  triple[,dependent_variable] <- as.factor(triple[,dependent_variable])
  rf_triple <- randomForest(as.formula(paste(names(triple)[dependent_variable], "~.")), data = triple, ntree=100, mtry=2, importance=TRUE)
  
  quadruple <- as.data.frame(quadruple)
  quadruple[,dependent_variable] <- as.factor(quadruple[,dependent_variable])
  rf_quadruple <- randomForest(as.formula(paste(names(quadruple)[dependent_variable], "~.")), data = quadruple, ntree=100, mtry=2, importance=TRUE)
  
  # predict and quality measurements # # AUC, PR, MCC #
  predict_double <- predict(rf_double, test[,-dependent_variable])
  misClasificError_rf_double[i] <- mean(predict_double != test[,dependent_variable])
  roc_double_rf <- tryCatch({pROC::roc(predict_double, test[,dependent_variable])},
                            error=function(cond){return(NA)})
  auc_rf_double[i] <- tryCatch({pROC::auc(roc_double_rf)[1]}, error=function(cond){return(NA)})
  aupr_rf_double[i] <- PRROC::pr.curve(predict_double, test[,dependent_variable])$auc.integral
  mcc_rf_double[i] <- mltools::mcc(predict_double, as.factor(test[,dependent_variable]))
  
  predict_triple <- predict(rf_triple, test[,-dependent_variable])
  misClasificError_rf_triple[i] <- mean(predict_triple != test[,dependent_variable])
  roc_triple_rf <- tryCatch({pROC::roc(predict_triple, test[,dependent_variable])},
                               error=function(cond){return(NA)})
  auc_rf_triple[i] <- tryCatch({pROC::auc(roc_triple_rf)[1]}, error=function(cond){return(NA)})
  aupr_rf_triple[i] <- PRROC::pr.curve(predict_triple, test[,dependent_variable])$auc.integral
  mcc_rf_triple[i] <- mltools::mcc(predict_triple, as.factor(test[,dependent_variable]))
  
  
  predict_quadruple <- predict(rf_quadruple, test[,-dependent_variable])
  misClasificError_rf_quadruple[i] <- mean(predict_quadruple != test[,dependent_variable])
  roc_quadruple_rf <- tryCatch({pROC::roc(predict_quadruple, test[,dependent_variable])},
                            error=function(cond){return(NA)})
  auc_rf_quadruple[i] <- tryCatch({pROC::auc(roc_quadruple_rf)[1]}, error=function(cond){return(NA)})
  aupr_rf_quadruple[i] <- PRROC::pr.curve(predict_quadruple, test[,dependent_variable])$auc.integral
  mcc_rf_quadruple[i] <- mltools::mcc(predict_quadruple, as.factor(test[,dependent_variable]))
...
models <- c(rep("LR", 3*50), rep("RF", 3*50))
method <- c(rep(c(rep("double",50), rep("triple",50), rep("quadruple",50)),2))
value <- c(1-misClasificError_lr_double, 1-misClasificError_lr_triple,
           1-misClasificError_lr_quadruple,
           1-misClasificError_rf_double, 1-misClasificError_rf_triple,
           1-misClasificError_rf_quadruple)
...
models <- c(rep("LR", 3*50), rep("RF", 3*50))
method <- c(rep(c(rep("double",50), rep("triple",50), rep("quadruple",50)),2))
value <- c(auc_lr_double, auc_lr_triple,
           auc_lr_quadruple,
           auc_rf_double, auc_rf_triple,
           auc_rf_quadruple)
...
auc_plot <- ggplot2::ggplot(auc, aes(fill=models, y=value, x=method)) + 
  geom_boxplot() + coord_cartesian(ylim=c(0.65,1)) +
...
models <- c(rep("LR", 3*50), rep("RF", 3*50))
method <- c(rep(c(rep("double",50), rep("triple",50), rep("quadruple",50)),2))
value <- c(aupr_lr_double, aupr_lr_triple,
           aupr_lr_quadruple,
           aupr_rf_double, aupr_rf_triple,
           aupr_rf_quadruple)
...
aupr_plot <- ggplot2::ggplot(aupr, aes(fill=models, y=value, x=method)) + 
  geom_boxplot() + coord_cartesian(ylim=c(0.965,0.9725)) +
...
models <- c(rep("LR", 3*50), rep("RF", 3*50))
method <- c(rep(c(rep("double",50), rep("triple",50), rep("quadruple",50)),2))
value <- c(mcc_lr_double, mcc_lr_triple,
           mcc_lr_quadruple,
           mcc_rf_double, mcc_rf_triple,
           mcc_rf_quadruple)
...
# accuracy
p_values <- c(1, t.test(1-misClasificError_lr_double, 1-misClasificError_lr_triple)$p.value,
              t.test(1-misClasificError_lr_double, 1-misClasificError_lr_quadruple)$p.value,
              1, t.test(1-misClasificError_rf_double, 1-misClasificError_rf_triple)$p.value,
              t.test(1-misClasificError_rf_double, 1-misClasificError_rf_quadruple)$p.value)
means = c(1-mean(na.omit(misClasificError_lr_double)), 1-mean(na.omit(misClasificError_lr_triple)),
          1-mean(na.omit(misClasificError_lr_quadruple)),
          1-mean(na.omit(misClasificError_rf_double)), 1-mean(na.omit(misClasificError_rf_triple)),
          1-mean(na.omit(misClasificError_rf_quadruple)))
models <- c(rep("LR", 3), rep("RF", 3))
method <- c(rep(c("double", "triple", "quadruple"),2))
...
# AUC
p_values <- c(1, t.test(auc_lr_double, auc_lr_triple)$p.value, 
              t.test(auc_lr_double, auc_lr_quadruple)$p.value,
              1, t.test(auc_rf_double, auc_rf_triple)$p.value,
              t.test(auc_rf_double, auc_rf_quadruple)$p.value)
means = c(mean(na.omit(auc_lr_double)), mean(na.omit(auc_lr_triple)),
          mean(na.omit(auc_lr_quadruple)),
          mean(na.omit(auc_rf_double)), mean(na.omit(auc_rf_triple)),
          mean(na.omit(auc_rf_quadruple)))
models <- c(rep("LR", 3), rep("RF", 3))
method <- c(rep(c("double", "triple", "quadruple"),2))
...
# AUPR
p_values <- c(1, t.test(aupr_lr_double, aupr_lr_triple)$p.value, 
              t.test(aupr_lr_double, aupr_lr_quadruple)$p.value,
              1, t.test(aupr_rf_double, aupr_rf_triple)$p.value,
              t.test(aupr_rf_double, aupr_rf_quadruple)$p.value)
means = c(mean(na.omit(aupr_lr_double)), mean(na.omit(aupr_lr_triple)),
          mean(na.omit(aupr_lr_quadruple)),
          mean(na.omit(aupr_rf_double)), mean(na.omit(aupr_rf_triple)),
          mean(na.omit(aupr_rf_quadruple)))
models <- c(rep("LR", 3), rep("RF", 3))
method <- c(rep(c("double", "triple", "quadruple"),2))
...
# MCC
p_values <- c(1, t.test(mcc_lr_double, mcc_lr_triple)$p.value, 
              t.test(mcc_lr_double, mcc_lr_quadruple)$p.value,
              1, t.test(mcc_rf_double, mcc_rf_triple)$p.value,
              t.test(mcc_rf_double, mcc_rf_quadruple)$p.value)
means = c(mean(na.omit(mcc_lr_double)), mean(na.omit(mcc_lr_triple)),
          mean(na.omit(mcc_lr_quadruple)),
          mean(na.omit(mcc_rf_double)), mean(na.omit(mcc_rf_triple)),
          mean(na.omit(mcc_rf_quadruple)))
models <- c(rep("LR", 3), rep("RF", 3))
method <- c(rep(c("double", "triple", "quadruple"),2))
